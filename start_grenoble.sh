...
RES=$(iotlab-experiment submit -n project-2 -d 480 -l 2,archi=m3:at86rf231+site=grenoble)
ID=$(echo $RES | jq '.id')
...
iotlab-node --flash firmware/sensor_grenoble.elf -l "grenoble,m3,${mynodes[1]}" -i "${ID}"
